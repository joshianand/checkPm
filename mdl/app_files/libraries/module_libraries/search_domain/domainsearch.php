...
class Domainsearch {
    ...
    protected function IsDomainAvailable($domain_name = '', $slogan_lefts = 300) {
        ...
        if ($CI->domain_model->IsDomainAvailable($domain_name) == FALSE) {
            return FALSE;
        } else {
            sleep(rand(3, 5));
            $query = "http://who.is/whois/$domain_name";
            $data = $this->scrape_request($query);
            $html = str_get_html($data);

            if ($html) {
                $info = $html->find('.registrar-information', 0);
                if ($info) {
...
//            }
     ...
     * @return string Returns scraped data
     */
